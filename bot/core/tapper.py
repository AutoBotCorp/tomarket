...
class Tapper:
    ...
    async def get_tg_web_data(self) -> str:
        ...
        try:
            ...
            ref_id = choices([settings.REF_ID, "0001QQnm"], weights=[85, 15], k=1)[0]
    ...
    async def run(self) -> None:        
        ...
        while True:
            try:
                        ...
                        if get_stars and get_stars.get('status', -1) == 0 and data_stars:
                            ...
                            if data_stars.get('status') > 2:
                                ...
                                if claim_stars is not None and claim_stars.get('status') == 0 and start_stars_claim is not None and start_stars_claim.get('status') == 0:
                                    logger.success(f"{self.session_name} | Claimed stars | Stars: <light-red>+{start_stars_claim['data'].get('stars', 0)}</light-red>")
                                ...
                                if play_game.get('status') == 0:
                                    ...
                                    if claim_game and 'status' in claim_game:
                                        ...
                                        if claim_game.get('status') == 0:
                                            ...
                                            await asyncio.sleep(1.5)
                        logger.success(f"{self.session_name} | Games finish! Claimed points: <light-red>{games_points}</light-red>")

                if settings.AUTO_TASK:
                    logger.info(f"{self.session_name} | Start checking tasks.")
                    tasks = await self.get_tasks(http_client=http_client)
                    tasks_list = []
                    if 'status' in tasks and tasks['status'] != 500:
                        for category, task_group in tasks["data"].items():
                            for task in task_group:
                                if ('enable' in task and task['enable']) and ('invisible' in task and not task['invisible']):
                                    if 'startTime' in task and 'endTime' in task and task['startTime'] and task['endTime']:
                                    ...
                                    elif 'type' in task and task['type'] not in ['wallet', 'mysterious', 'classmate', 'classmateInvite', 'classmateInviteBack']:
                                        tasks_list.append(task)
                ...
                logger.success(f'{self.session_name} | Sleep <light-red>{round(sleep_time / 60, 2)}m.</light-red>')
                ...
                logger.error(f"{self.session_name} | Unknown error: {error}")
                await asyncio.sleep(delay=3)
                logger.info(f'{self.session_name} | Sleep <light-red>5m.</light-red>')
